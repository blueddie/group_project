...
# 162~ 177
# AUTOTUNE : 병렬처리
# 메모리 사용 최적화, 속도 향상: 데이터셋의 각 요소를 동시에 처리함으로써 데이터 로딩 및 전처리에 걸리는 시간을 대폭 줄일 수 있고 이는 특히 큰 이미지 데이터셋을 다룰 때 중요하며,
# 모델 학습 시간을 단축
# 학습 과정의 안정성
...
# sequence to sequence(seq2seq)     encoder -> decoder의 구조       대표적인 예) 번역기
# 인코더 : 입력문장의 모든 단어들을 순차적으로 입력받은뒤 마지막에 오든 단어 정보들을 압축해서 하나의 벡터로 만든다. -> (context vector)
# 입력 문장의 정보가 하나의 context vector로 모두 압축되면 인코더는 context vector를 디코더로 전송한다.
# 디코더 : 받은 context vector를 번역해 단어를 한개씩 순차적으로 출력한다.
#  seq2seq에서 사용되는 모든 단어들은 임베딩 벡터로 변환 후 입력으로 사용


# 인코더를 자세히보면, 입력 문장은 단어 토큰화를 통해서 단어 단위로 쪼개지고 단어 토큰 각각은 RNN 셀의 각 시점의 입력이 된다. 
# 인코더 RNN 셀은 모든 단어를 입력받은 뒤에 인코더 RNN 셀의 마지막 시점의 은닉 상태를 디코더 RNN 셀로 넘겨주는데 이를 컨텍스트 벡터라고 함.
# 컨텍스트 벡터는 디코더 RNN 셀의 첫번째 은닉 상태에 사용

# 디코더는 초기 입력으로 문장의 시작을 의미하는 심볼 <start>가 들어갑니다. 디코더는 <start>가 입력되면, 다음에 등장할 확률이 높은 단어를 예측
# 디코더는 이런 식으로 기본적으로 다음에 올 단어를 예측하고, 그 예측한 단어를 다음 시점의 RNN 셀의 입력으로 넣는 행위를 반복합니다. 
# 이 행위는 문장의 끝을 의미하는 심볼인 <end>가 다음 단어로 예측될 때까지 반복
# 디코더의 모든 출력값은 소프트맥스 함수를 통과시켜야한다. 단어별로 예측된 확률값 중 가장 높은 놈이 출력으로 나와야하기 때문.

# 이 seq2seq모델에는 두가지의 큰 문제가 있다.
# 첫째, 하나의 고정된 크기의 벡터에 모든 정보를 압축하려고 하니까 정보손실이 발생한다.
# 둘째, RNN의 고질적인 문제인 기울기 소실 문제가 존재한다.
